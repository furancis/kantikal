...
The Kantikal is sealf'salve's private lyric-first music generation workstation. The app treats lyrics as the source of creative intent, then uses liked sounds, voices, mixes, references, seeds, models, and lineage to steer music generations toward that intent.

`Kantikal` is the repo/internal implementation name. It is not the user-facing brand.
...
The interface should feel like opening a private songbook, room, machine, and ritual object for sealf'salve to make songs. It should not feel like a generic dashboard, node toy, DAW clone, startup product, SaaS tool, or thin provider wrapper.
