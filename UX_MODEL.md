...
Internal implementation/repo name may remain `Kantikal`.
...
Builds the next provider run from:
...
4. A generation desk that assembles the next provider prompt/run from lyric intent, Sound DNA, taste decisions, provider constraints, and lineage.
...
1. provider access path: direct API, browser automation, or manual import/export must be chosen from the live available path. If no stable write path exists, generation actions become prepared briefs plus import slots until access is verified.
...
6. Provider contract drift: provider surfaces, limits, and capabilities can change. Provider actions must be wrapped behind adapters and surfaced as verified/unverified.
