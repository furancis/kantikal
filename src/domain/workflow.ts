...
export type providerWorkflow = BriefInput & {
  stage: WorkflowStage
...
export function createWorkflow(input: BriefInput): providerWorkflow {
  return {
    ...input,
    stage: 'brief-ready',
...
export function selectTrack(workflow: providerWorkflow, trackId: string): providerWorkflow {
...
export function openSongLab(workflow: providerWorkflow): providerWorkflow {
...
export function lockSongLabRegion(workflow: providerWorkflow, sectionId: string): providerWorkflow {
  if (!workflow.songLab) {
...
export function openMusicVideoLane(workflow: providerWorkflow): providerWorkflow {
  if (!workflow.selectedTrack) {
...
export function queueMusicVideoRender(workflow: providerWorkflow): providerWorkflow {
  if (!workflow.musicVideoLane) {
...
export function mergeMusicVideoRuntimeWorkflow(
  current: providerWorkflow,
  runtimeWorkflow: providerWorkflow,
): providerWorkflow {
...
export function queueLipsyncRepair(workflow: providerWorkflow): providerWorkflow {
  if (!workflow.musicVideoLane) {
...
export function applyArchiveFirstCleanup(workflow: providerWorkflow): providerWorkflow {
  if (!workflow.generationBatch) {
...
export function restoreArchivedTracks(workflow: providerWorkflow): providerWorkflow {
  if (!workflow.generationBatch) {
...
export function analyzeTrackGenealogy(workflow: providerWorkflow): TrackGenealogy {
  const selectedTrack = workflow.selectedTrack
...
function toReleasePackItems(workflow: providerWorkflow, includeVideo: boolean): ReleasePackItem[] {
  const selectedTrack = workflow.selectedTrack
  if (!selectedTrack) {
    return []
  }
...
function toReleasePackReceipts(workflow: providerWorkflow, includeVideo: boolean): ProvenanceReceipt[] {
  const selectedTrack = workflow.selectedTrack
  if (!selectedTrack) {
    return []
  }
...
function trackGenealogyAncestors(workflow: providerWorkflow): TrackGenealogyAncestor[] {
  return [
    {
      id: 'brief',
      label: 'Brief DNA',
...
function trackGenealogyDescendants(workflow: providerWorkflow): TrackGenealogyDescendant[] {
...
function trackGenealogyInheritedTraits(workflow: providerWorkflow): InheritedTrait[] {
...
function branchFitScore(workflow: providerWorkflow, track: GeneratedTrack): BranchFitScore {
  const rating = workflow.taste.ratings[track.id]
...
function deadBranchForTrack(workflow: providerWorkflow, trackId: string): DeadBranch | null {
...
function ensureTrackInActiveBatch(workflow: providerWorkflow, trackId: string): GeneratedTrack {
  const track = workflow.generationBatch?.tracks.find((candidate) => candidate.id === trackId)
  if (!track) {
    throw new Error(`Track ${trackId} is not in the active generation batch`)
  }
...
function isVideoOutputApproved(workflow: providerWorkflow, output: ProviderTaskOutput): boolean {
...
function createProviderJob(workflow: providerWorkflow, result: ProviderJobResultInput): ProviderJob {
  return {
...
function assetIdsForWorkflow(workflow: providerWorkflow): string[] {
