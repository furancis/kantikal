...
import type { MusicProvider } from './api/provider'
...
function persistedWorkflowFixture(trackPrefix = 'hydrated'): providerWorkflow {
...
describe('The Kantikal shell', () => {
  it('keeps the music video lane subordinate to the song workflow', () => {
    ...
    expect(screen.getByRole('heading', { name: /Generate batch/i })).toBeInTheDocument()
    expect(screen.getByLabelText(/runtime status/i)).toHaveTextContent(/Provider runtime/i)
  ...
  it('creates editable workflow objects from the visual app', async () => {
    ...
    expect(screen.getByText(/Full provider capability map/i)).toBeInTheDocument()
  ...
  it('sends a liked-track taste lock to the provider before generation', async () => {
    const user = userEvent.setup()
    const requests: Array<Parameters<MusicProvider['generateBatch']>[0]> = []
  ...
  it('keeps the current release pack when a replacement generation fails', async () => {
    const user = userEvent.setup()
    let calls = 0
    const flakyProvider: MusicProvider = {
      async generateBatch() {
        calls += 1
        if (calls === 1) {
          return {
  ...
  it('keeps the current release pack when a replacement batch is rejected', async () => {
    const user = userEvent.setup()
    let calls = 0
    const emptyBatchProvider: MusicProvider = {
      async generateBatch() {
        calls += 1
        if (calls === 1) {
          return {
  ...
  it('preserves provider action updates that land while lipsync QA is in flight', async () => {
    const user = userEvent.setup()
    let resolveLipsync: (workflow: providerWorkflow) => void = () => undefined
    const lipsyncPromise = new Promise<providerWorkflow>((resolve) => {
  ...
  it('opens a different recent project without leaking the previous project state', async () => {
    ...
    const rejectingProvider: MusicProvider = {
      async generateBatch() {
  ...
  it('locks prompt and generation controls while the music video lane is open', async () => {
    ...
    expect(screen.getByRole('button', { name: /Generate batch/i })).toBeDisabled()
