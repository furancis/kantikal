...
Printing Press / logged-in provider web is the primary provider execution path. The `providerapi.org` surface in this document is an optional compatible-provider adapter map, not the official provider product contract, and it must be treated as disabled until a real credential probe succeeds.

All provider capabilities must be mapped here before implementation is called complete. This ledger is sourced from the current provider documentation index plus product guardrails; it is an enforcement target, not proof that real provider calls are complete.

| Capability | UI Surface | Backend Owner | Status | Auth Boundary | Adapter Action |
| --- | --- | --- | --- | --- | --- |
| Create song | Creation Canvas batch node | provider adapter | Implemented | Server | generateBatch |
| Custom mode | Prompt/lyrics inspector | provider adapter | Implemented | Server | generateBatch |
| Instrumental | Prompt/lyrics inspector | provider adapter | Implemented | Server | generateBatch |
| Lyrics generation | Lyrics card | provider adapter | Implemented | Server | generateLyrics |
| Model/version selection | Prompt inspector | provider adapter | Implemented | Server | selectModelVersion |
...
| Extend | Song Lab timeline | provider adapter | Implemented | Server | extendTrack |
| Upload and extend audio | Song Lab upload rail | provider adapter | Implemented | Server | uploadAndExtend |
| Cover | Song Lab action rail | provider adapter | Implemented | Server | coverTrack |
| Upload and cover audio | Song Lab upload rail | provider adapter | Implemented | Server | uploadAndCover |
| Remix/recreate | Song Lab version branch | provider adapter | Unsupported | Server | remixTrack |
| Remaster | Song Lab version branch | provider adapter | Unsupported | Server | remasterTrack |
| Replace section | Song Lab region editor | provider adapter | Implemented | Server | replaceSection |
| Add instrumental | Song Lab stem composer | provider adapter | Implemented | Server | addInstrumental |
| Add vocals | Song Lab stem composer | provider adapter | Implemented | Server | addVocals |
| Boost music style | Style stack action | provider adapter | Implemented | Server | boostMusicStyle |
| Generate mashup | Song Lab mashup lane | provider adapter | Implemented | Server | generateMashup |
| Generate sounds | Sound design card | provider adapter | Implemented | Server | generateSounds |
| Stems | Stem cards | provider adapter | Implemented | Server | separateStems |
| MIDI from audio | Stem cards | provider adapter | Implemented | Server | generateMidi |
...
| Timestamped lyrics | Lyrics card timeline | provider adapter | Implemented | Server | getTimestampedLyrics |
...
| Provider music video creation | MV lane | provider adapter | Implemented | Server | createProviderMusicVideo |
...
- `https://docs.providerapi.org/llms.txt`
- `https://docs.providerapi.org/provider-api/provider-api.json`
- `https://docs.providerapi.org/file-upload-api/file-upload-api.json`
- `https://docs.providerapi.org/provider-api/provider-voice-api.json`
- `https://docs.providerapi.org/provider-api/quickstart`
- `https://docs.providerapi.org/provider-api/generate-music`
- `https://docs.providerapi.org/provider-api/extend-music`
- `https://docs.providerapi.org/provider-api/upload-and-cover-audio`
- `https://docs.providerapi.org/provider-api/upload-and-extend-audio`
...
- Real provider dispatch is isolated in `server/musicApiAdapter.ts`; it requires `runtime: "server"` and a server-side API key before any network call.
- Endpoint-backed actions build typed provider payloads from current compatible-provider specs and block before fetch when required fields are missing.
