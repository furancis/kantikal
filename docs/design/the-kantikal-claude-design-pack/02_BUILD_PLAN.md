...
`lyrics -> intent map -> Sound DNA -> Suno generation brief -> takes -> granular taste decisions -> branch genealogy -> Song Lab -> release pack`
...
5. Generation Desk
   - Shows exact next Suno brief before execution.
...
- Suno execution path must be verified live.
