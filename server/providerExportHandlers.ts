...
import { normalizeProviderCallback } from './musicApiAdapter'
...
export function createProviderExportHandlers(input: {
  ...
  async function persist(projectId: string, workflow: providerWorkflow): Promise<ProviderExportHandlerResult> {
    const state = await store.save(projectId, exportSnapshotFromWorkflow(projectId, workflow))
    return { projectId, workflow, state }
  }

  async function workflowWithStoredState(projectId: string, workflow: providerWorkflow): Promise<providerWorkflow> {
