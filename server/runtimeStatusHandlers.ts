...
const MusicProviderModes = new Set(['live', 'local'])
...
export function createRuntimeStatusHandlers(input: RuntimeStatusHandlersInput = {}) {
  ...
  return {
    async getStatus(): Promise<RuntimeStatus> {
      const apiBaseUrl = env.MUSIC_PROVIDER_API_BASE_URL ?? 'https://api.providerapi.org'
      const apiV1BaseUrl = `${apiBaseUrl.replace(/\/$/, '')}/api/v1`
      const uploadBaseUrl = env.MUSIC_PROVIDER_UPLOAD_BASE_URL ?? 'https://providerapiorg.redpandaai.co'
      const comfyBaseUrl = env.COMFYUI_BASE_URL ?? defaultComfyBaseUrl
      const modelRoot = env.COMFYUI_MODEL_ROOT ?? defaultModelRoot
      const providerMode = MusicProviderMode(env.MUSIC_PROVIDER_MODE, env.MUSIC_PROVIDER_API_KEY)
      const models = await listModelFiles(modelRoot, readDir)
      const provider = await probeprovider(fetchImpl, {
        apiBaseUrl,
        apiV1BaseUrl,
        uploadBaseUrl,
        providerMode,
        apiKey: env.MUSIC_PROVIDER_API_KEY,
        callbackUrl: env.MUSIC_PROVIDER_CALLBACK_URL,
      })
...
async function probeprovider(fetchImpl: typeof fetch, input: providerProbeInput): Promise<RuntimeStatus['provider']> {
  const base = {
  ...
  if (input.providerMode === 'local') {
    return {
      ...
      message: input.apiKey
        ? 'Printing Press remains the primary provider web lane; optional compatible-provider adapter has a key but local QA mode is active.'
        : 'Printing Press / logged-in provider web is the primary lane; optional compatible-provider adapter is disabled until a valid key proves live.',
    }
  }
  ...
  if (!input.apiKey) {
    return {
      ...
      state: 'blocked',
      message:
        'Optional compatible-provider adapter is blocked because live mode needs MUSIC_PROVIDER_API_KEY; Printing Press remains primary.',
    }
  }
  ...
  try {
    ...
    return {
      ...
      message: providerCode
        ? `Optional compatible-provider adapter remaining-credits probe returned HTTP ${response.status} with provider code ${providerCode}.`
        : `Optional compatible-provider adapter remaining-credits probe returned HTTP ${response.status}.`,
    }
  } catch {
    return {
      ...base,
      state: 'configured',
      message:
        'Optional compatible-provider adapter key is configured, but the live remaining-credits probe did not complete.',
    }
  }
}
...
function MusicProviderMode(value: string | undefined, apiKey: string | undefined): 'live' | 'local' {
  if (value && MusicProviderModes.has(value)) {
