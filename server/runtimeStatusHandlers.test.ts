...
const musicProviderApiKeyName = 'MUSIC_PROVIDER_API_KEY'
...
describe('runtime status handlers', () => {
  ...
  it('marks live provider credentials online only after the server-side credit probe succeeds', async () => {
    const handlers = createRuntimeStatusHandlers({
      env: {
        [musicProviderApiKeyName]: 'server-secret',
        MUSIC_PROVIDER_MODE: 'live',
  ...
  it('does not treat HTTP 200 as live provider success when the provider body rejects auth', async () => {
    const handlers = createRuntimeStatusHandlers({
      env: {
        [musicProviderApiKeyName]: 'server-secret',
        MUSIC_PROVIDER_MODE: 'live',
    ...
    await expect(handlers.getStatus()).resolves.toMatchObject({
      provider: {
        ...
        providerMode: 'live',
        credential: 'present',
        message: 'Optional compatible-provider adapter remaining-credits probe returned HTTP 200 with provider code 401.',
      },
    })
  })

  it('leaves live provider state configured when the credit probe returns a non-auth provider error', async () => {
    const handlers = createRuntimeStatusHandlers({
      env: {
        [musicProviderApiKeyName]: 'server-secret',
        MUSIC_PROVIDER_MODE: 'live',
    ...
    await expect(handlers.getStatus()).resolves.toMatchObject({
      provider: {
        ...
        providerMode: 'live',
        credential: 'present',
        message: 'Optional compatible-provider adapter remaining-credits probe returned HTTP 200 with provider code 500.',
      },
    })
  })
...
async function makeModelRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'provider-runtime-models-'))
  tempRoots.push(root)
  return root
}
