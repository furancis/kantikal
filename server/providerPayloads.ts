...
const defaultUploadPath = 'kantikal/uploads'
...
export function buildProviderActionPayload(input: ProviderPayloadBuildInput): ProviderPayloadBuildResult {
  ...
  switch (definition.action) {
    ...
    case 'coverTrack':
      // provider's upload-cover endpoint is the documented cover path and requires a provider-visible uploadUrl.
      return requireFields(
        {
          uploadUrl: uploadUrl(source),
          ...baseMusic(),
      ...
      return requireFields(
        {
          ...
          audioId: audioId(source),
          name: stringValue(source.name) ?? requiredText(request.voice, 'Kantikal Persona'),
    ...
    case 'checkVoiceAvailability':
      // provider's voice/check-voice schema uses task_id here, unlike the other task-based endpoints.
    ...
    case 'regenerateVoiceValidationPhrase':
      // provider's voice/regenerate schema documents calBackUrl; accept normal aliases but send the provider field.
      return requireFields(
        {
          taskId: taskId(source),
...
function promptText(request: ExecuteProviderActionRequest): string {
  ...
  return requiredText(payloadPrompt ?? request.lyrics ?? request.brief, 'Kantikal generation')
}

function titleText(request: ExecuteProviderActionRequest): string {
  return requiredText(stringValue(request.payload?.title) ?? request.brief, 'Kantikal')
}
